...
- Encoding: AEAD seal with a 32-byte store key (`TicketKeySize`) using the ticket ID as **associated data**. Format: `ticket_id (16)` || `aead_output`, where `aead_output = nonce (12) || ciphertext || tag`. The nonce is auto-generated by `AEAD.Seal`; the ticket ID is not used for nonce derivation.
